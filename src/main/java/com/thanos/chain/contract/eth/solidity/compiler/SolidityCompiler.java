...
public class SolidityCompiler {
    ...
    public SolidityCompiler(SystemConfig config) {
    ...
    public static Result compile(File sourceDirectory, boolean combinedJson, Option... options) throws IOException {
        return getInstance().compileSrc(sourceDirectory, true, combinedJson, options);
    }
    ...
    private List<String> prepareCommandOptions(boolean optimize, boolean combinedJson, Option... options) throws IOException {
        ...
        if (optimize) {
            ...
            //commandParts.add("--optimize-runs=0");
        }
        if (combinedJson) {
    ...
    public static SolidityCompiler getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new SolidityCompiler(ConfigResourceUtil.loadSystemConfig());
        }
        return INSTANCE;
    }
}