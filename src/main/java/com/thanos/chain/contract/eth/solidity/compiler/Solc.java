 ...
 */
package com.thanos.chain.contract.eth.solidity.compiler;
...
public class Solc {
    ...
    Solc(SystemConfig config) {
        try {
            init(config);
        } catch (IOException e) {
            throw new RuntimeException("Can't init solc resolver: ", e);
        }
    }

    private void init(SystemConfig config) throws IOException {
        if (config != null && config.customSolcPath() != null) {
            solc = new File(config.customSolcPath());
            if (!solc.canExecute()) {
                throw new RuntimeException(String.format(
                        "Solidity resolver from config solc.path: %s is not a valid executable",
    ...
    private String getOS() {
        ...
        if (osName.contains("win")) {
            ...
            throw new RuntimeException("Can't find solc resolver: unrecognized OS: " + osName);
        }
    }
