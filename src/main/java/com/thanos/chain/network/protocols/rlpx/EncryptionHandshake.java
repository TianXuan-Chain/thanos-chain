...
public class EncryptionHandshake {
    ...
    public EncryptionHandshake(ECPoint remotePublicKey) {
        ...
        ephemeralKey = new ECKeyOld(random);
        initiatorNonce = new byte[NONCE_SIZE];
        random.nextBytes(initiatorNonce);
        isInitiator = true;
    }

    EncryptionHandshake(ECPoint remotePublicKey, ECKeyOld ephemeralKey, byte[] initiatorNonce, byte[] responderNonce, boolean isInitiator) {
        this.remotePublicKey = remotePublicKey;
     ...
     */
    public AuthInitiateMessageV4 createAuthInitiateV4(ECKeyOld key) {
        ...
        message.signature = ephemeralKey.signECDSA(signed);
    ...
    public AuthInitiateMessageV4 decryptAuthInitiateV4(byte[] in, ECKeyOld myKey) throws InvalidCipherTextException {
        try {
    ...
    public AuthResponseMessageV4 decryptAuthResponseV4(byte[] in, ECKeyOld myKey) {
        try {
    ...
    AuthResponseMessageV4 makeAuthInitiateV4(AuthInitiateMessageV4 initiate, ECKeyOld key) {
        ...
        ECKeyOld ephemeral = ECKeyOld.recoverFromSignature(recIdFromSignatureV(initiate.signature.v),
    ...
    public AuthResponseMessageV4 handleAuthResponseV4(ECKeyOld myKey, byte[] initiatePacket, byte[] responsePacket) {
     ...
     * @param key our private key
     */
    public AuthInitiateMessage createAuthInitiate(@Nullable byte[] token, ECKeyOld key) {
        ...
        message.ephemeralPublicHash = sha3(ephemeralKey.doGetPubKey(), 1, 64);
        message.publicKey = key.getPubKeyPoint();
        message.nonce = initiatorNonce;
        return message;
    }
    ...
    public AuthResponseMessage decryptAuthResponse(byte[] ciphertext, ECKeyOld myKey) {
        try {
    ...
    public AuthInitiateMessage decryptAuthInitiate(byte[] ciphertext, ECKeyOld myKey) throws InvalidCipherTextException {
        try {
            byte[] plaintext = ECIESCoder.decrypt(myKey.getPrivKey(), ciphertext);
    ...
    public AuthResponseMessage handleAuthResponse(ECKeyOld myKey, byte[] initiatePacket, byte[] responsePacket) {
...
//        System.out.println("ecdhe " + Hex.toHexString(agreedSecret));
    ...
    public byte[] handleAuthInitiate(byte[] initiatePacket, ECKeyOld key) throws InvalidCipherTextException {
    ...
    AuthResponseMessage makeAuthInitiate(byte[] initiatePacket, ECKeyOld key) throws InvalidCipherTextException {
        AuthInitiateMessage initiate = decryptAuthInitiate(initiatePacket, key);
        return makeAuthInitiate(initiate, key);
    }

    AuthResponseMessage makeAuthInitiate(AuthInitiateMessage initiate, ECKeyOld key) {
        initiatorNonce = initiate.nonce;
        remotePublicKey = initiate.publicKey;
        BigInteger secretScalar = key.keyAgreement(remotePublicKey);
        byte[] token = ByteUtil.bigIntegerToBytes(secretScalar, NONCE_SIZE);
