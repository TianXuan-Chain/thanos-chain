...
public class MessageCodec extends MessageToMessageCodec<FrameCodec.Frame, Message> {

    private static final Logger logger = LoggerFactory.getLogger("network");

    @Override
    protected void decode(ChannelHandlerContext ctx, FrameCodec.Frame frame, List<Object> out) throws Exception {
        Message msg;
        byte[] payload = new byte[frame.getContentSize()];
        ByteStreams.read(frame.getStream(), payload, 0, frame.getContentSize());
        msg = new Message(frame.getType(), frame.getCode(), frame.getRemoteType(), frame.getMsgId(), frame.getNodeId(), payload);
        out.add(msg);
    }
    ...
    protected void encode(ChannelHandlerContext ctx, Message msg, List<Object> out) throws Exception {
        ...
        FrameCodec.Frame frame = new FrameCodec.Frame(msg.getType(), msg.getCode(), msg.getRemoteType(), msg.getRpcId(), msg.getNodeId(), msg.getEncoded());
