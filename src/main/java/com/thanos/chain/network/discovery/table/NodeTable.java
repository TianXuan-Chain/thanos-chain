 ...
 */
package com.thanos.chain.network.discovery.table;
...
public class NodeTable {
    ...
    {
        nodes = new ArrayList<>();
        buckets = new NodeBucket[NtreeOptions.BINS];
        for (int i = 0; i < NtreeOptions.BINS; i++)
        {
...
//        boolean res = nodes.remove(node);
    ...
    public synchronized List<Node> getClosestNodes(byte[] targetId) {
        ...
        if (closestEntries.size() > NtreeOptions.BUCKET_SIZE) {
            closestEntries = closestEntries.subList(0, NtreeOptions.BUCKET_SIZE);
        }
