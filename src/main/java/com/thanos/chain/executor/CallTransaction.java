    ...
    public static class Contract {
        ...
        private Function getBySignatureHash(byte[] hash) {
            if (hash.length == 4 ) {
                ...
                throw new RuntimeException("Function signature getHash should be 4 or 32 bytes length");
            }
            return null;
        }
         ...
         * Finds corresponding event by its signature getHash and thus is not applicable to
         * anonymous events
         */
            ...
            for (Param input : event.inputs) {
                if (input.indexed) {
                    ...
                    if (input.type.isDynamicType()) {
                        ...
                        // the Keccak-256 getHash of it is stored as topic instead.
